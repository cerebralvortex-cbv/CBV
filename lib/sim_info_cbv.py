...
import functools
import ctypes
from ctypes import c_int32, c_float, c_wchar,  c_char
...
class SPageFilePhysicsCBV_FuelCalc(ctypes.Structure):
    _pack_ = 4
    _fields_ = [
        ('packetId', c_int32),
        ('gas', c_float),
        ...
        # More info is availiable but not needed in this app

    ]


class SPageFileGraphicCBV_FuelCalc(ctypes.Structure):
    _pack_ = 4
    _fields_ = [
        ('packetId', c_int32),
        ('status', AC_STATUS),
...
class SPageFileStaticCBV_FuelCalc(ctypes.Structure):
    _pack_ = 4
    _fields_ = [
...
class CBV_FuelCalcShared:
    def __init__(self):
        self._acpmf_physics_cbv_fuelcalc = mmap.mmap(0, ctypes.sizeof(SPageFilePhysicsCBV_FuelCalc), "acpmf_physics")
        self._acpmf_graphics_cbv_fuelcalc = mmap.mmap(0, ctypes.sizeof(SPageFileGraphicCBV_FuelCalc), "acpmf_graphics")
        self._acpmf_static_cbv_fuelcalc = mmap.mmap(0, ctypes.sizeof(SPageFileStaticCBV_FuelCalc), "acpmf_static")
        self.physics = SPageFilePhysicsCBV_FuelCalc.from_buffer(self._acpmf_physics_cbv_fuelcalc)
        self.graphics = SPageFileGraphicCBV_FuelCalc.from_buffer(self._acpmf_graphics_cbv_fuelcalc)
        self.static = SPageFileStaticCBV_FuelCalc.from_buffer(self._acpmf_static_cbv_fuelcalc)

    def close(self):
        self._acpmf_physics_cbv_fuelcalc.close()
        self._acpmf_graphics_cbv_fuelcalc.close()
        self._acpmf_static_cbv_fuelcalc.close()

    def __del__(self):
        self.close()

sm = CBV_FuelCalcShared()

def demo():
    import time

    for _ in range(400):
        print(sm.static.track, sm.graphics.tyreCompound, sm.graphics.currentTime,
              sm.physics.rpms, sm.graphics.currentTime, sm.static.maxRpm, list(sm.physics.tyreWear))
        time.sleep(0.1)


def do_test():
    for struct in sm.static, sm.graphics, sm.physics:
